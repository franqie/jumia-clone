...
const AppProvider = ({children}) => {
  ...
  const addItem = (id)=>{
    const newItem = data.find((item)=>item.productId === id);
  ...
  const removeItem = (id)=>{
    const newItem = cart.filter((item)=>item.productId !== id);
    setCart(newItem);
  ...
  const changeQty = (e, id)=>{
    ...
    const newCart = cart.map((item)=>{
      ...
      if(item.productId === id && btn.contains("dec-btn")  && item.quantity > 1){
        setAlert({ show: true, msg: "Item quantity has been updated" });
        return { ...item, quantity: item.quantity - 1 }
      } else if(item.productId === id && btn.contains("inc-btn")){
        setAlert({ show: true, msg: "Product added successfully" });
        return { ...item, quantity: item.quantity + 1 }
      }
